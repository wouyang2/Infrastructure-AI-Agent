...
from agents.helpers.scheduling_precedent_tool import (
    SchedulingPrecedentTool,
    required_crew_for_plan,
)
from models import (
    InspectionCase,
    MaintenancePlan,
...
class SchedulingAgent:
    ...
    def schedule(
        ...
        scheduling_precedents: list[dict[str, Any]] | None = None,
    ) -> RepairSchedule:
        if scheduling_precedents is None:
            scheduling_precedents = SchedulingPrecedentTool(self.retriever).invoke(
    ...
    def _required_crew(self, maintenance_plan: MaintenancePlan) -> str | None:
        return required_crew_for_plan(maintenance_plan)
